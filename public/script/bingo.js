...
function showColorOptions(tileElement) {
  ...
  // Check if the color is in hex format directly
  if (currentColor.startsWith('#')) {
    currentColorKey = currentColor;
  } else {
    // Convert RGB color to hex
    currentColorKey = rgbToHex(currentColor);
  }

  const currentColorIndex = colorOptions.indexOf(currentColorKey);

  // If currentColor is not found, default to grey index
...
function setTileCompletion(tileElement, isCompleted, updateButton = true) {
  ...
  if (isCompleted) {
    ...
    const savedState = JSON.parse(localStorage.getItem('bingoState')) || {};
    tileElement.style.backgroundColor = savedState[tileElement.dataset.tileName]?.color || '#3a3a3a';
